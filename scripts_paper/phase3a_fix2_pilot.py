#!/usr/bin/env python3
"""Phase 3A-FIX2: correct OOF label alignment + residual units in PG-MT-BCR.

Corrects two Phase-3A-FIX selection bugs:
  F11 - inner OOF validation truth was taken from training labels (yB) instead
        of the validation fold's own labels (yC);
  F12 - inner OOF BCR residual predictions stayed in standardized units while
        the final outer model de-standardized, so alpha/hyperparameters were
        selected in one unit system and applied in another.

Architecture, optimizer, grids, priors, and gates are unchanged.
Fresh development seeds: [4343, 4444, 4545, 4646].
"""
...
OUT = ROOT / "outputs" / "iclr" / "palf_phase3a_fix2_pg_mt_bcr"
...
def run_outer_split(FC, SC, y_wm, y_fi, r0, controls, p_wm, p_fi,
                    ...
                    alignment_records=None, parity_probe=None):
    t0 = time.time()
    inner_folds = partition(train_idx, seed, f"outer{fold}", N_INNER)
    if alignment_records is None:
        alignment_records = []

    # ── Precompute inner R0 + standardized matrices (FIX2 §7) ──
    inner = []
    for fi, (B, C) in enumerate(inner_folds):
        ...
        mu_WM, sd_WM = res_WM_B.mean(), res_WM_B.std()
        mu_FI, sd_FI = res_FI_B.mean(), res_FI_B.std()
        st = {"B": B, "C": C, "fcB": fcB, "scB": scB, "fcC": fcC, "scC": scC,
              "base_WM_C": base["WM"], "base_FI_C": base["FI"],
              "yC_WM": y_wm[C], "yC_FI": y_fi[C],
              "res_WM_B": res_WM_B, "res_FI_B": res_FI_B,
              "z_WM_B": (res_WM_B - mu_WM) / (sd_WM + 1e-12),
              "z_FI_B": (res_FI_B - mu_FI) / (sd_FI + 1e-12),
              "mu_WM": mu_WM, "sd_WM": sd_WM,
              "mu_FI": mu_FI, "sd_FI": sd_FI,
              "fi": fi}
        inner.append(st)

    # ── Concatenated inner OOF truth/base arrays aligned to train_idx (FIX2 §8) ──
    nT = len(train_idx)
    ...
    for st in inner:
        for g, v in zip(st["C"], st["base_WM_C"]):
            base_oof_WM[pos[int(g)]] = v
        for g, v in zip(st["C"], st["base_FI_C"]):
            base_oof_FI[pos[int(g)]] = v
        # F11 fix: validation truth comes from the validation fold's own labels
        for g, v in zip(st["C"], st["yC_WM"]):
            y_oof_WM[pos[int(g)]] = v
        for g, v in zip(st["C"], st["yC_FI"]):
            y_oof_FI[pos[int(g)]] = v

    # Mandatory hard gates (FIX2 §8)
    assert np.all(np.isfinite(y_oof_WM)) and np.all(np.isfinite(y_oof_FI))
    assert np.all(np.isfinite(base_oof_WM)) and np.all(np.isfinite(base_oof_FI))
    assert np.array_equal(y_oof_WM, y_wm[train_idx]), "F11: y_oof_WM misaligned"
    assert np.array_equal(y_oof_FI, y_fi[train_idx]), "F11: y_oof_FI misaligned"

    inner_base_r_WM = metrics(y_oof_WM, base_oof_WM)["pearson"]
    inner_base_r_FI = metrics(y_oof_FI, base_oof_FI)["pearson"]

    # Diagnostic record for units_alignment_audit.json
    alignment_records.append({
        "seed": seed, "fold": fold, "n_train": nT,
        "y_oof_equals_yT": True,
        "inner_base_r_WM": float(inner_base_r_WM),
        "inner_base_r_FI": float(inner_base_r_FI),
    ...
    def _fit_residuals(st, kind, lam_amp, lam_prior, prior_sh, prior_wm, prior_fi,
        ...
        return train_bcr(st["fcB"], st["scB"],
                         st["z_WM_B"] if kind in ("MT", "ST_WM") else None,
                         st["z_FI_B"] if kind in ("MT", "ST_FI") else None,
                         prior_sh, prior_wm, prior_fi, lam_amp, lam_prior,
                         kind=kind, seed=seed, diagnostics=opt_diag,
                         diag_context=ctx)

    def _oof_residuals_raw(kind, lam_amp, lam_prior, prior_sh, prior_wm, prior_fi,
                           tag, extra):
        """Concatenated OOF residual predictions in RAW target units (FIX2 §9).

        predict_bcr returns standardized (z-scale) residual predictions; each
        inner fold de-standardizes with its own B-residual statistics before
        storage, so selection and the final outer refit share one unit system.
        """
        ...
        for fi, st in enumerate(inner):
            ...
            rWM_z, rFI_z = predict_bcr(r, st["fcC"], st["scC"])
            # F12 fix: de-standardize per-fold before concatenation
            rWM_raw = (rWM_z * st["sd_WM"] + st["mu_WM"]
                       if rWM_z is not None else None)
            rFI_raw = (rFI_z * st["sd_FI"] + st["mu_FI"]
                       if rFI_z is not None else None)
            if rWM_raw is not None:
                for g, v in zip(st["C"], rWM_raw):
                    res["WM"][pos[int(g)]] = v
            if rFI_raw is not None:
                for g, v in zip(st["C"], rFI_raw):
                    res["FI"][pos[int(g)]] = v

            # FIX2 §10: dimensional-consistency check on a deterministic sample
            if parity_probe is not None:
                parity_probe(st, rWM_z, rFI_z, rWM_raw, rFI_raw)
        return res
    ...
    for mid, (psh, pwm, pfi, use_prior) in prior_sets.items():
        ...
        for la in LAMBDA_AMP_GRID:
            for lp in lp_grid:
                res = _oof_residuals_raw("MT", la, lp, psh, pwm, pfi,
                                         f"{mid}_la{la}_lp{lp}",
                                         {"lambda_amp": la, "lambda_prior": lp})
                res_sd = {"WM": float(res["WM"].std()), "FI": float(res["FI"].std())}
                for aW in ALPHA:
                    for aF in ALPHA:
                        cands.append({"lambda_amp": la, "lambda_prior": lp,
                                      ...
                                      "res_sd_WM": res_sd["WM"],
                                      "res_sd_FI": res_sd["FI"],
                                      "residual_prediction_units": RESIDUAL_PREDICTION_UNITS,
                                      "model": mid})
    ...
    for mid, use_prior in [("B0", False), ("B1", True)]:
        ...
        for task in ["WM", "FI"]:
            ...
            for la in LAMBDA_AMP_GRID:
                for lp in lp_grid:
                    res = _oof_residuals_raw(kind, la, lp, psh_use, pwm_use, pfi_use,
                                             f"{mid}_{task}_la{la}_lp{lp}",
                                             {"lambda_amp": la, "lambda_prior": lp})
                    resid = res[task]
                    res_sd = float(resid.std())
                    for a in ALPHA:
                        cands.append({"lambda_amp": la, "lambda_prior": lp, "alpha": a,
                                      "res": resid, "res_sd": res_sd,
                                      "residual_prediction_units": RESIDUAL_PREDICTION_UNITS,
                                      "model": mid, "task": task})
            if task == "WM":
...
def make_plots(df, decomp, stab, faith):
    ...
    fig.tight_layout(); fig.savefig(plots / "fig_phase3a_fix2_model_comparison.pdf")
    fig.savefig(plots / "fig_phase3a_fix2_model_comparison.png"); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(plots / "fig_phase3a_fix2_seed_deltas.pdf")
    fig.savefig(plots / "fig_phase3a_fix2_seed_deltas.png"); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(plots / "fig_phase3a_fix2_prior_effect.pdf")
    fig.savefig(plots / "fig_phase3a_fix2_prior_effect.png"); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(plots / "fig_phase3a_fix2_multitask_effect.pdf")
    fig.savefig(plots / "fig_phase3a_fix2_multitask_effect.png"); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(plots / "fig_phase3a_fix2_factor_stability.pdf")
    fig.savefig(plots / "fig_phase3a_fix2_factor_stability.png"); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(plots / "fig_phase3a_fix2_biomarker_faithfulness.pdf")
    fig.savefig(plots / "fig_phase3a_fix2_biomarker_faithfulness.png"); plt.close(fig)
...
def main():
    ...
    if not audit_pass:
        ...
        with open(OUT / "COMPLETE", "w") as f:
            f.write("PHASE3A_FIX2_BASELINE_AUDIT_FAILED\n")
    ...
    for seed in seeds:
        ...
        for fold in range(N_OUTER):
            ...
            if ck.exists():
                ...
                def _parity_probe(st, rWM_z, rFI_z, rWM_raw, rFI_raw,
                                  _seed=seed, _fold=fold):
                    """FIX2 §10: verify the real helper path stores raw units and
                    that base + alpha*(z*sd+mu) matches base + alpha*raw."""
                    errs = []
                    for rz, rw_raw, sd, mu, base_c in [
                        (rWM_z, rWM_raw, st["sd_WM"], st["mu_WM"], st["base_WM_C"]),
                        (rFI_z, rFI_raw, st["sd_FI"], st["mu_FI"], st["base_FI_C"]),
                    ]:
                        if rz is None:
                            continue
                        # stored OOF residual must equal de-standardized prediction
                        errs.append(float(np.max(np.abs(rw_raw - (rz * sd + mu)))))
                        # final-formula parity for alpha in {0, 0.5, 1}
                        for alpha in (0.0, 0.5, 1.0):
                            lhs = base_c[:3] + alpha * rw_raw[:3]
                            rhs = base_c[:3] + alpha * (rz[:3] * sd + mu)
                            errs.append(float(np.max(np.abs(lhs - rhs))))
                    max_err = float(max(errs))
                    parity_records.append({"seed": _seed, "fold": _fold,
                                           "fi": st["fi"], "max_err": max_err})
                    assert max_err <= 1e-12, f"parity error {max_err}"
    ...
    # ── units_alignment_audit.json (FIX2 §20) ──
    al = [r for o in split_rows for r in o.get("alignment_records", [])]
    units_audit = {
        "residual_prediction_units": RESIDUAL_PREDICTION_UNITS,
        "n_alignment_checks": len(al),
        "all_y_oof_equal_yT": bool(all(r["y_oof_equals_yT"] for r in al)),
        "inner_base_r_WM_values": [float(r["inner_base_r_WM"]) for r in al],
        "inner_base_r_FI_values": [float(r["inner_base_r_FI"]) for r in al],
        "inner_base_r_WM_mean": float(np.mean([r["inner_base_r_WM"] for r in al])) if al else None,
        "inner_base_r_FI_mean": float(np.mean([r["inner_base_r_FI"] for r in al])) if al else None,
        "parity_max_error": float(max([r["max_err"] for r in parity_records])) if parity_records else None,
        "n_parity_checks": len(parity_records),
    }
    with open(OUT / "units_alignment_audit.json", "w") as f:
        json.dump(units_audit, f, indent=2)
    print(f"units_alignment_audit: {len(al)} alignment checks, "
          f"parity max err = {units_audit['parity_max_error']}")
    ...
    # ── Selection details (FIX2 §15 columns) ──
    seld = []
    for out in split_rows:
        ibrw = out.get("inner_base_r_WM")
        ibrf = out.get("inner_base_r_FI")
        for key, (best, scored) in out["selection"].items():
            for s in scored:
                ...
                row = {"seed": out["seed"], "fold": out["fold"], "model": key,
                       ...
                       "inner_base_r_WM": ibrw, "inner_base_r_FI": ibrf,
                       "residual_prediction_units": s.get("residual_prediction_units"),
                       "eligible": s.get("eligible", False),
                       "selected": s.get("selected", False),
                       "fisher": s.get("fisher", None)}
                if "mW" in s:
                    row["candidate_r_WM"] = s["mW"]["pearson"]
                    row["candidate_delta_WM"] = s["dW"]
                    row["candidate_rmse_WM"] = s["mW"]["rmse"]
                    row["candidate_residual_sd_WM_raw"] = s.get("res_sd_WM")
                if "mF" in s:
                    row["candidate_r_FI"] = s["mF"]["pearson"]
                    row["candidate_delta_FI"] = s["dF"]
                    row["candidate_rmse_FI"] = s["mF"]["rmse"]
                    row["candidate_residual_sd_FI_raw"] = s.get("res_sd_FI")
                if "m" in s:
                    row[f"candidate_r_{task}"] = s["m"]["pearson"]
                    row[f"candidate_delta_{task}"] = s["d"]
                    row[f"candidate_rmse_{task}"] = s["m"]["rmse"]
                    row[f"candidate_residual_sd_{task}_raw"] = s.get("res_sd")
                seld.append(row)
    sdf = pd.DataFrame(seld)
    sdf.to_csv(OUT / "selection_details.csv", index=False)
    # Mandatory FIX2 assertion: every row must carry raw-target units
    units = sdf.get("residual_prediction_units")
    assert units is not None and (units == RESIDUAL_PREDICTION_UNITS).all(), \
        "selection_details contains non-raw-unit residual predictions"
    print(f"selection_details: {len(sdf)} rows, all units == '{RESIDUAL_PREDICTION_UNITS}'")
    ...
    print(f"\nPHASE3A_FIX2_DECISION: {decision}")
