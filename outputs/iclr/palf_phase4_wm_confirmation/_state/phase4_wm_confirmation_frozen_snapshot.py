...
def linear_final_map(model, r0_info):
    ...
    W = r0_info["w_r0"].copy()
    c = r0_info["c_r0"]
    if model["expert_type"] is not None and model["alpha"] > 0:
...
def stage_holdout():
    ...
    # Verify stored scalers equal the shared development scalers
    for name, m in models.items():
        if name == "R0":
            continue
        assert np.allclose(m["fc_final"]["scaler_mean"], fc_scaler.mean_, atol=1e-9)
        assert np.allclose(m["sc_final"]["scaler_mean"], sc_scaler.mean_, atol=1e-9)
